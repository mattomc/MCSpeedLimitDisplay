...
local speedlimit = '##'
local enabled = true
local speedosl = true -- Boolean to allow members to change the in game layout to just show their speed
local posX = 0.175
local posY = 0.835
...
function text(content)
    ...
    SetTextEntry("STRING")
    if speedosl then
        AddTextComponentString(content .. " MPH / ~r~" .. speedlimit .. " MPH ")
    else
...
RegisterNetEvent( "speedo:Toggle" )
AddEventHandler("speedo:Toggle", function(player)
    if IsPedInAnyVehicle(GetPlayerPed(player), false) then
       if enabled == true then
        enabled = false
        notify('~g~[MC Speedo] ~w~The speedo has been disabled, to renable it please utilize the command /speedo')
       else
        enabled = true
        notify('~g~[MC Speedo] ~w~The speedo has been renabled!')
       end
    else
        notify('~r~[MC Speedo] ~w~To Disable the speedo script please enter a vehicle')
    end
end)

RegisterNetEvent('speedo:togglespeedlimit')
AddEventHandler('speedo:togglespeedlimit', function(player)
    if IsPedInAnyVehicle(GetPlayerPed(player), false) then
        if speedosl == true then
            speedosl = false
            notify('~g~[MC Speedo] ~w~The speedo will not show the speedlimt')
        else
            speedosl = true
            notify('~g~[MC Speedo] ~w~The speedo will now show the speedlimt')
        end
     else
         notify('~r~[MC Speedo] ~w~To change the setting on the speedo script please enter a vehicle')
     end
end)

RegisterCommand("speedo", function(source, args)
    TriggerEvent('speedo:Toggle', source)
end)

RegisterCommand("speedlimit", function(source, args)
    TriggerEvent('speedo:togglespeedlimit', source)
end)

Citizen.CreateThread(function()
    TriggerEvent("chat:addSuggestion", '/speedo', 'Enables/Disabled the Speedo and the speed limit script (Please note this is for the current game)')
end)
Citizen.CreateThread(function()
    TriggerEvent("chat:addSuggestion", '/speedlimit', 'Disables/Displays the current speedlimit on the road you are traveling')
end)
